...
export function RhLaunchStudio({ handle }: { handle: string | null }) {
  ...
  return (
    <form onSubmit={(event) => void launch(event)} className="pad-fade space-y-5 rounded-3xl border border-white/10 p-5">
      <div>
        <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-white/40">Robinhood Chain · 4663 · live</p>
        <h2 className="mt-1 text-2xl font-semibold">Pons launch</h2>
        <p className="mt-2 text-sm text-white/55">
          Robinhood launches are fully live and out of beta. Pons v2. Curve is live at create so anyone can buy
          and sell. Volume feeds the book and graduates into a locked Uniswap v4 LP. Your in-app RH wallet pays
          gas and takes creator fees.
        </p>
      </div>
