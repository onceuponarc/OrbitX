...
import { DEPLOY_STAGES, useCustomLaunch } from "@/components/custom-launch/draft-provider";
import { cn } from "@/lib/utils";

export function DeploymentProgress() {
  const { deployStage, deployError } = useCustomLaunch();
  ...
  return (
    <section className="ox-console relative overflow-hidden rounded-[1.5rem] p-5 lg:p-7">
      ...
      <div className="relative">
        <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-gold/80">On-chain deployment</p>
        <h2 className="mt-1 text-3xl font-semibold tracking-tight">Deploying Custom Launch</h2>
        <p className="mt-2 max-w-xl text-sm text-white/50">
          Waiting for a confirmed chain transaction. This screen does not invent a token address.
        </p>
        ...
        {deployError ? <p className="mt-4 text-sm text-heat">{deployError}</p> : null}
      </div>
    </section>
  );
}
