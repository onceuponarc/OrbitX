...
export function EvmTrade({
  ...
  async function submit() {
    if (!signedIn) {
      setError("Sign in with X first. Trades use your in-app wallet.");
      return;
    }
  ...
  return <div className="space-y-3 rounded-3xl border border-white/10 p-5">
    ...
    <p className="text-xs text-white/45">{side === "buy" ? `${quoteLabel} in` : `${symbol} tokens in`} · signed by your in-app desk. No browser wallet.</p>
    {!signedIn ? (
      <Button type="button" asChild><a href="/auth/login">Sign in with X to trade</a></Button>
    ) : (
