...
import { tradeOnArcPad } from "@/lib/arc/arcpad-trade";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await tradeOnArcPad({ userId: user.id, token: body.token as `0x${string}`, side: body.side, amountUi: body.amount }));
  } catch (error) {
